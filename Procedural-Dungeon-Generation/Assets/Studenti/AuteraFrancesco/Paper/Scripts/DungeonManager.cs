...
namespace paper
{
    public class DungeonManager : MonoBehaviour
    {
        ...
        private void AssignEnemyMaxRoom(AiEnemy enemy) {
            foreach (HallWay h in hallWays) {
                h.enemy = enemy;
            }
            foreach (Room r in ChangeRooms.resizedRooms) {
               
                r.enemy = enemy;
            }
        }
        private void AssignEnemyRangeRoom(AiEnemy enemy)
        {
            foreach (HallWay h in hallWays)
            {
                h.enemy = enemy;
        ...
        IEnumerator waitBeforeBake()
        {
            ...
            PlayerCheckPosition checkPositionPlayer = FindObjectOfType<PlayerCheckPosition>();
            checkPositionPlayer.SetLastRoom(createdRooms[createdRooms.Count - 1]);
