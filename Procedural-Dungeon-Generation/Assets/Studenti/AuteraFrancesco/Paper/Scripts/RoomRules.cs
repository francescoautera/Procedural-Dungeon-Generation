...
namespace paper
{
    ...
    public class RoomRules : ScriptableObject
    {
        ...
        public AiEnemy AddEnemy(Room room,Transform posPlayer) {
            
                var en = Instantiate(enemy, room.center.transform.position, Quaternion.identity);
                en.GetComponentInChildren<DirectedAgent>().SetTarget(posPlayer.position);
                return en.GetComponent<AiEnemy>();
